...
Ces systèmes reprennent les principes d'architecture multi-tiers utilisés sur le Web avec les contenus représentés par des fichiers ou des flux binaires, d'où leur appellation de \e{file-based production system}.
% 
...
\begin{liste}
	...
	\item \pc{Memnon} qui est spécialisée dans la numérisation, la documentation et l'archivage de contenus audio et vidéo.

	\item \pc{Perfect Memory} qui s'est créée pendant le projet afin d'accompagner les solutions du projet sur le marché grand public et prospecte également le marché professionnel.
...
Elle nous permet également de préciser la manière dont nous posons le problème, à la fois en termes métiers et avec nos lunettes de scientifiques.}
...
Le chapitre \g{\ref{chap:omod}. Outils de modélisation} commence par clarifier les besoins en modélisations à partir d'un scénario de production collaborative impliquant des acteurs professionnels et amateurs (\ref{sec:cdcf}).
...
Ainsi, nous examinons les définitions des concepts de \gui{systèmes d'organisation de connaissances} (SOC) et en particulier les relations qu'entretiennent \gui{terminologie} et \gui{ontologie} (\ref{sec:defs}).
...
\e{La dernière partie de ce mémoire montre comment notre contribution est utilisée dans le cadre du projet MediaMap et ouvre la discussion sur ce travail de thèse.}

Le chapitre \g{\ref{chap:app}. Applications et expérimentations} introduit les diverses applications qui ont été développées (\ref{sec:app}) par nos partenaires et les expérimentations qu'elles ont permis de mener (\ref{sec:xp}).
