...
Nous proposons donc de définir la terminologie suivante pour distinguer trois niveaux successifs de réutilisation, chacun visant à créer un nouveau document mais suivant des opérations différentes (voir Figure \ref{img:intro:reuse}). 
...
\begin{liste}
	\item Le \eg{retraitement} (repurposing) qui se caractérise par une automatisation de la transformation opérée sur le contenu (quelque soit son type), c'est-à-dire que cette transformation est effectuée par un logiciel lui-même paramétré par un humain. 
...
Du côté des fournisseurs de contenus, il existe une distinction entre les chaînes du fabricant et du fournisseur du contenu :  

\begin{liste}
	\item \e{Déléguer la fabrication à des contributeurs tiers} : l'utilisation du scripting pour définir la \pc{Commande} de contenu attendue semble une solution satisfaisante, à condition que le vocabulaire utilisé soit normé et rattaché à une conceptualisation de manière à éviter la confusion ou les différences d'interprétations. 
...
\begin{liste}
	...
	Finalement, plusieurs \pc{Finition}s sont à envisager suivant les cas d'exploitation.
\end{liste}
...
Ces métadonnées sont structurées par le schéma \pc{Descriptive Metadata Scheme-1} (DMS-1) que nous présenterons en détails dans la suite de la section. 
...
S'il s'appliquait au \pc{File Package}, les informations correspondraient aux informations de création (format d'image original).
...
\begin{liste} 
	\item En tant que résultat d'une production qu'il faut valoriser commercialement (\pc{Production}).
	\item Décomposé en éléments narratif (plan, scène) faisant partie d'un ensemble documentaire (\pc{Scene}) et dont on décrit le contexte historique (\pc{Production}) et les évènements réels (\pc{Clip}) ou fictifs (\pc{Scene}) qui s'y déroulent.
	\item Matériel audiovisuel construit pendant la production dont on décrit les caractéristiques techniques (\pc{Clip}).
\end{liste}
