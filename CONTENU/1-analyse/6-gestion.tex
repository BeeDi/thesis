\newpage
\section{Circulation et réutilisation des objets audiovisuels(m)}\label{sec:gest}
...
L'intérêt de ces formats est de constituer un objet numérique qui structure l'enregistrement du contenu et de ces informations annexes et fournit ainsi une manière unique d'y accéder (\cite{Ferreira2010}).
Sans cela, les informations seraient dispersées et le lien avec le contenu devrait se faire via des références. 
Cette force constitue également un inconvénient lorsque le modèle d'information à associer au contenu est figé et ne correspond pas à ses besoins.
...
Leur particularité est de pouvoir intégrer d'utiliser des schémas de métadonnées propres aux besoins de l'industrie, mais aussi de pouvoir en utiliser d'autres. 
Comme ces formats servent de référence à l'industrie, il est particulièrement intéressant pour nous de comprendre leur modélisation d'un objet audiovisuel, de voir comment ils gèrent les résultats intermédiaires de la chaîne de production ou quelles genres d'informations ils embarquent avec le contenu.
...
% objectif
Son objectif est de favoriser les échanges de contenus audio-visuels finis en les associant à d'autres données et métadonnées (\cite{Devlin2002}). 
% portée et usage


% description
Ses principales caractéristiques en tant que format sont les suivantes (\cite{Ferreira2010a}) : 
\begin{liste} 
	...
	Comme nous l'avons vu, un fichier MXF contient des métadonnées propres à l'ensemble du fichier ainsi que des métadonnées propres à chaque paquet de données (Package). 
	Ces éléments sont intégrés dans la structure du fichier, au même titre que le contenu.
\end{liste}
