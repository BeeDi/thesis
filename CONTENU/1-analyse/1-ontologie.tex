...
\newpage
\section{Les Systèmes d'Organisation de Connaissances}\label{sec:defs}
%Distinguer entre terme et concept ; dictionnaire, thésaurus, ontologies etc.

\e{
L'objectif de cette section est de clarifier ce qui appartient au domaine de la  linguistique et ce qui relève du domaine conceptuel, en vue d'identifier les notions qui nous serviront à spécifier une solution au cahier des charges dressé dans la section précédente. 
...
\e{
À cela \citeauthor{Zacklad2010} souhaitent ajouter des modes de description du contenu émergents ou plus faiblement codifiés comme par exemple les folksonomies. 
Les SOC qui nous intéressent en particulier sont les schèmes structurés par types de relations, c'est-à-dire les thésaurus, les terminologies et les ontologies (\ref{sec:tto}). 
Pour les ontologies, nous en distinguerons différents types et présenterons une méthode de construction qui permet d'éclairer la distinction entre une modélisation et sa représentation informatique (\ref{sec:construction}).
Nous finirons par examiner la question de la validation des ontologies fortement impliquées dans un contexte métier ou un domaine particulier (\ref{sec:valid-ic}).}
...
Il s'agit d'un ensemble de mots structurés et reliés suivant leur \e{signification}, c'est-à-dire leur sens normé ou commun à plusieurs contextes d'usages particuliers (à l'inverse du sens, qui lui varie suivant les usages, \cite{Roche2005}). 
...
\ciel{Une ontologie est une spécification explicite d'une conceptualisation.}

 La définition proposée par \cite{Uschold1996} (traduite par \citeauthor{Charlet2002}) nous permet de préciser la composition d'une conceptualisation et la spécification qui en est faite par une ontologie  : 
...
\begin{listeni}
	\item L'\g{objectif} de l'ontologie (\ciel{purpose}) où l'on distingue entre deux objectifs, servir de référence ou bien être utilisée dans un cas d'application :
	\begin{liste}
		\item L'\e{ontologie de référence} qui vise à établir un consensus entre des agents (humains, machines) d'une même communauté, ou bien à servir d'explication et de langage commun avec des agents de communautés différentes. 
	...
	\item L'\g{expressivité} de l'ontologie (\ciel{expressiveness}) où l'on considère un engagement plus ou moins fort sur le formalisme de représentation :
	\begin{liste}
		\item L'\e{ontologie légère} (\ciel{lightweight}) qui peut se limiter à une hiérarchie de concepts bien connus dans une communauté avec quelques relations. 
		L'apport se situe alors dans la structuration des connaissances qui clarifie leur signification plus qu'elle ne les établit.
		\item L'\e{ontologie lourde} (\ciel{heavyweight}) qui vise à exclure toute ambiguïté terminologique et conceptuelle.
		Pour cela, la formalisation se veut beaucoup plus contrainte et détaillée afin de forcer une interprétation. 
	\end{liste}

	\item La \g{spécificité} de l'ontologie qui peut se limiter à un domaine ou s'étendre à un ensemble de domaines voire plusieurs champs disciplinaires : 
	\begin{liste}
		\item L'\e{ontologie générique} (\ciel{generic}, \ciel{upper/top level}) qui contient des concepts utilisés dans de nombreux champs disciplinaires (évènements, processus etc.)
		\item L'\e{ontologie noyau} (\ciel{core}) qui comporte des concepts qui se situent à la croisée de plusieurs domaines. 
		La distinction avec une ontologie générique se fait car elle comporte des concepts utilisables quel que soit le domaine.
		...
		\item L'\e{ontologie de domaine} qui contient des concepts propres à un domaine, et bien souvent des éléments plus génériques extraits de domaines différents.
