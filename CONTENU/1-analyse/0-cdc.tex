...
D'une part, nous avons des practiciens professionnels utilisant un ou plusieurs vocabulaires métierse suffisamment définis pour que l'on puisse en faire des dictionnaires (\cite{Journot2008}; \cite{Pinel2008}). 
...
Ainsi, l'écart entre collaborateurs (qu'ils soient tous professionnels, ou un mélange d'amateurs et de professionnels) peut se situer à différents niveaux : 
\begin{liste} 
...
Le terme est utilisé en Europe en rappel à son emploi caractéristique dans les films américains des années 1910-1940, notamment dans les westerns où le plan permettait de montrer le pistolet à la ceinture des personnages\footnote{Roger Boussinot, l'Encyclopédie du Cinéma, Bordas.}. 
Ce cadrage est aussi appelé \gui{plan 3/4} et en anglais \ciel{3/4 shot}, \ciel{medium-long shot} ou \ciel{american shot} pour traduire l'expression popularisée en Europe. 
...
Cet objectif nous amène à considérer la construction d'une ressource termino-ontologique. L'on\-to\-lo\-gie permet de représenter les concepts partagés par les professionels de la réalisation audiovisuelle et la terminologie permet de capturer les différentes formes d'expression associées à ces concepts. 
...
\begin{enumerate}
	\item[(\e{$\delta_1$}]\e{: multi-jargon}) la variabilité des pratiques linguistiques des organisations et des communautés implique d'associer plusieurs termes à un même concept. 
	...
	\item[(\e{$\delta_2$}]\e{: documentation}) la variabilité de compréhension des communautés implique d'associer des explications (chaîne lexicale) ou des illustrations (ressource média) aux concepts afin d'en enrichir la documentation. 
	
	\item[(\e{$\delta_3$}]\e{: gestion, évolution}) la variabilité des cas de collaboration implique de pouvoir étendre la conceptualisation initiale ou la terminologie pour s'adapter à de nouvelles pratiques ou de nouvelles communautés. 
	Cela implique une gestion et une évolution indépendante de l'ontologie et de la terminologie. 
\end{enumerate}
\end{problemes}

Dans le cas d'une demande de cadrage en plan américain, la demande est d'abord exprimée dans le jargon de la RTBF puis traduite dans le jargon de la VRT (plan américain pour la RTBF, plan 3/4 pour la VRT) [\g{$\delta_1$ : multi-jargon}]. 
Ensuite, pour les amateurs, la terminologie est enrichie par des illustrations [\g{$\delta_2$ : documentation}]. 
Enfin, un nouveau concept de cadrage est ajouté (plan américain large ou plan moyen serré) [\g{$\delta_3$ : gestion, évolution}] en vue d'une nouvelle coopération avec la VRT. 
De plus, le problème de la langue (français et flamand) s'ajoute à la question des jargons métiers. 
