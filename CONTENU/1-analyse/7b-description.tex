%=============================
%=============================
\subsection{Modèles de description \e{in situ}}\label{sec:plan}
...
\e{
Des modèles de description, dit \e{in situ}, sont apparus récemment avec pour approche de modéliser les connaissances contenues dans les documents de la chaîne de production audiovisuelle.
...
Ce point de vue de description particulier tranche avec des modélisations qui se concentre sur le matériel audiovisuel [\g{B1 : autonomie}] mais rend de ce fait difficile leur association [\g{B2 : réutilisabilité}].
...
\paragraph{Discussion}
L'intérêt de cette modélisation est d'associer des connaissances sur la production, c'est-à-dire comment a été construit le film, et le résultat de cette production, le film en tant que tel [\g{B2 : réutilisabilité}].
Ceci est rendu possible par le fait que la structure d'annotation est celle du script, qui correspond à la fois à une unité de contenu -- le film -- et une unité de travail pour la production [\g{B1 : autonomie}].
...
\subsection{Modèles de description \e{a posteriori}}\label{sec:post}
\e{
...
\paragraph{Discussion}
Concernant la modélisation des objets audiovisuels, MPEG-7 et les ontologies réutilisant le Structure DS décomposent très finement le matériel audiovisuel dans l'espace et le temps [\g{B1 : autonomie}].
Les \pc{Segments} ainsi constitués renvoient alors au premier des trois niveaux de modélisation de MPEG-7, à savoir \pc{ContentEntity}, \pc{MediaProfile} et \pc{MediaInstance}.
...
En tant que CE, chacun des \pc{Segment} d'une décomposition peut-être décrit par les schémas de descriptions et le schéma Semantic DS [\g{B2 : réutilisabilité}].
...
Le schéma \pc{Semantic} esquisse une description visant des entités représentés dans le matériel audiovisuel (objet, évènement, lieux, agent) et esquisse ainsi une ouverture à des descriptions propres à un domaine.
...
La modélisation des objets audiovisuels que nous avons spécifié dans notre cahier des charges (\ref{sec:cdc-av}) demande de distinguer les produits intermédiaires de la chaîne, afin de favoriser leur circulation et leur intégration dans d'autres chaînes de production [\g{B1 : autonomie}].
...
À l'inverse, les modèles de description \e{in situ} que nous avons étudié modélisent les objets audiovisuels en devenir, et décrivent justement la structure documentaire auctoriale qui guidera également la production du matériel audiovisuel.
Cette approche apparaît alors comme complémentaire par rapport aux modèles \e{a posteriori}, dans le sens où la structure documentaire prévu devrait être associé et synchronisé au matériel audiovisuel effectivement produit.\\
...
Les modèles de description \e{in situ} mettent également en avant les connaissances mobilisées par les contributeurs de la chaîne de production [\g{B2 : réutilisabilité}].
