...
La \pc{Dublin Core Metadata Initiative} (DCMI) est une organisation qui vise à développer et promouvoir l'usage de standards de description de ressources afin de faciliter l'échange d'information.
Depuis la première conférence organisé en 1995 à Dublin, Ohio, l'organisation a enrichi son standard original, le \pc{Dublic Core Metadata Element Set} (DCMES) de nouveaux éléments. 
En effet, la version étendue (\pc{DCMI Metadata Terms}) compte 54 éléments décomposés en propriétés et sous-propriétés, dont les 15 éléments d'origines (marqués en \e{italique} dans la Table \ref{tab:dcmi}).
L'intérêt de cette structuration est de pouvoir traiter les valeurs des sous-propriétés comme des valeurs de leur propriété parente. 
Le schéma est structuré de telle manière que ce remplacement conserve une signification, même si celle-ci est moins précise. 
...
\begin{table}[htb!]
   \begin{center}
		\begin{tabularx}{0.75\textwidth}{l X}
		   \hline
\gpc{Propriété} & \gpc{Sous-propriétés} \\ \hline\hline
accrualMethod & \\ \hline
